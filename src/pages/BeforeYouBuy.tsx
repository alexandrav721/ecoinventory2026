      ...
      <div className="container max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl">
            Check your inventory and your community before adding something new. The most sustainable purchase is the one you don't make.
          </p>
        </div>
        ...
        <Card className="p-6 mb-6">
          <Tabs value={mode} onValueChange={(v) => setMode(v as any)}>
            ...
            <TabsContent value="ai" className="space-y-3">
              <Textarea
                placeholder="e.g. I'm thinking about getting a stand mixer for baking on weekends. Probably the KitchenAid Artisan."
                value={input}
                onChange={(e) => setInput(e.target.value)}
                rows={4}
              />
            ...
            <Card className={`p-6 border-2 ${reco.color}`}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <h2 className="font-display text-2xl font-semibold mb-2">{result.product.product_name}</h2>
            ...
            {(result.moneySaved > 0 || result.co2Saved > 0) && (
              <div className="grid grid-cols-2 gap-4">
                <Card className="p-5">
                  ...
                  <div className="font-display text-3xl font-semibold">${result.moneySaved.toFixed(0)}</div>
                </Card>
                <Card className="p-5">
                  <div className="flex items-center gap-2 text-muted-foreground mb-1 text-sm">
                    <Leaf className="w-4 h-4" /> CO₂ avoided
                  </div>
                  <div className="font-display text-3xl font-semibold">{result.co2Saved.toFixed(1)} kg</div>
                </Card>
              </div>
            )}
